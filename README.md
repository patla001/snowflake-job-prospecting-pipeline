...
| Transform (next) | **dbt-snowflake** ([`dbt/`](dbt/README.md)) | 4 models (`dim_freeway`, `dim_station` via snapshot, `fact_traffic_hour`, `agg_traffic_daily`) + 18 tests, builds side-by-side in `DBT_DEV_MARTS` |
| Airflow ↔ dbt | **`pems_dbt_build` DAG** ([`dags/pems_dbt_build_dag.py`](dags/pems_dbt_build_dag.py)) | Separate DAG running `dbt snapshot + build + test` in an isolated venv baked into the image |
